...
const handleCancelToken = async () => {
  ...
  try {
    await fetch(
      `http://localhost:5000/api/tokens/cancel/Admission/${token.tokenNumber}`,
      {
        method: "PUT",
...
const handleGenerate = async () => {
  ...
  const serviceName = "Admission"; // ✅ define it
  ...
  try {
    ...
    setToken(data.token);
    localStorage.setItem("AdmissionToken", JSON.stringify(data.token));
    setShow(true);
  } catch (err) {
